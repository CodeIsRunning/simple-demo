...
public class RedisUtil {
    ...
    @Resource(name = "redisUtilTemplate")
        ...
        try {
            if (key != null && key.length > 0) {
                if (key.length == 1) {
                    ...
                    List<String> arrayList = new ArrayList<String>(key.length);
                    Collections.addAll(arrayList, key);
    ...
    public boolean hmset2(String key, Map<?, ?> m) {
        try {
     ...
     * @return
     */
    public boolean lmSet(String key, List<Object> value) {
        try {
            redisTemplate.opsForList().rightPushAll(key, value);
            return true;
        } catch (Exception e) {
            log.error("lmSet-error", e);
            RedisConnectionUtils.unbindConnection(redisTemplate.getConnectionFactory());
            return false;
        }
    }
     ...
     * @return
     */
    public boolean lmSet(String key, List<Object> value, long time) {
        try {
            redisTemplate.opsForList().rightPushAll(key, value);
            if (time > 0) {
                expire(key, time);
